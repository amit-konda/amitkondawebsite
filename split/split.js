...
function demoBill(id){const participants=[{id:"p1",name:"Alex",userId:id==="demo-2"?"demo-user":"payer-user",paymentStatus:"paid",amountCents:4832},{id:"p2",name:"Maya",userId:id==="demo-1"||id==="demo-3"?"demo-user":"maya-user",paymentStatus:id==="demo-3"?"confirmed":"unpaid",amountCents:3982,itemSubtotalCents:3200,taxCents:256,tipCents:526,finalAmountCents:3982},{id:"p3",name:"Sam",userId:"sam-user",paymentStatus:"reported_paid",amountCents:2600}];return {...demoDraft(),id,status:id==="demo-3"?"settled":"open",organizerUserId:id==="demo-2"?"demo-user":"payer-user",participants,currentParticipant:id==="demo-1"||id==="demo-3"?participants[1]:undefined};}
...
async function retryInvite(event,participantId,b){setBusy(event.currentTarget,true,"Retrying…");try{const result=await api(`/participants/${encodeURIComponent(participantId)}/retry-invite`,{method:"POST"});notice(result.delivery?.failed?"Invite queued for another retry.":"Invite resent.",result.delivery?.failed?"error":"success");billView(b.id);}catch(error){notice(error.message,"error");setBusy(event.currentTarget,false);}}
