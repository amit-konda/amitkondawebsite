...
function renderBlackjackLedger() {
  ...
  body.innerHTML = head + ledger.rows.map((r) => `<div class="ledger-row"><div class="lr-name">${esc(r.name)}${r.isViewer ? '<span class="you-tag">you</span>' : ""}</div><div class="lr-net money ${moneyClass(r.netCents)}">${esc(formatCents(r.netCents))}</div><div class="lr-meta">${r.sessionsPlayed} ${r.sessionsPlayed === 1 ? "session" : "sessions"} · last played ${esc(r.lastPlayedAt ? formatDate(r.lastPlayedAt) : "never")}</div></div>`).join("") + total;
}
