...
function itemEditor(x,i) { const lowConfidence = Number(x.ocrConfidenceBasisPoints ?? 10000) < 7500; return `<div class="item-row${lowConfidence ? " low-confidence" : ""}" data-item-id="${esc(x.id || "")}"><input class="input item-name" value="${esc(x.description || x.name)}" aria-label="Item ${i+1} name" maxlength="160"><span class="input-prefix"><input class="input money-input item-price" value="${(Number(x.lineTotalCents ?? x.totalCents ?? x.priceCents ?? 0)/100).toFixed(2)}" inputmode="decimal" aria-label="Item ${i+1} price"></span>${lowConfidence ? `<span class="ocr-flag">Check scan</span>` : ""}<button class="icon-btn" type="button" data-action="remove-item" aria-label="Remove item">×</button></div>`; }
