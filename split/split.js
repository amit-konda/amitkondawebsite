...
function organizerStatus(people,b){const canLock=b.status==="open"&&people.length&&people.every(p=>p.selectionStatus==="complete");return `<p class="eyebrow">Payment status</p><h2>Who’s settled up</h2><div class="participant-list">${people.map(p=>{const name=p.displayName||p.name||"Guest",status=p.paymentStatus||"unpaid",invite=p.invitationStatus||"pending",inviteNote=invite==="failed"?"Invite failed":invite==="queued"?"Invite queued":invite==="sent"||invite==="viewed"||invite==="accepted"?"Invite sent":"Invite pending";return `<div class="person"><span class="person-dot">${esc(initials(name))}</span><span class="person-copy"><strong>${esc(name)}</strong><small>${p.finalAmountCents!=null?money(p.finalAmountCents):esc((p.selectionStatus||"waiting").replaceAll("_"," "))} · ${esc(inviteNote)}</small></span>${invite==="failed"&&["open","locked"].includes(b.status)?`<button class="btn btn-sm" type="button" data-action="retry-invite" data-participant="${esc(p.id)}">Retry invite</button>`:b.status==="locked"&&status!=="confirmed"?`<button class="btn btn-sm" type="button" data-action="confirm-payment" data-participant="${esc(p.id)}">${status==="reported_paid"?"Confirm":"Mark paid"}</button>`:`<span class="status ${esc(status)}">${esc(status.replaceAll("_"," "))}</span>`}</div>`;}).join("")||`<p class="muted">Waiting for guests.</p>`}</div>${b.status==="open"?`<button class="btn btn-dark btn-block" type="button" data-action="lock-bill" ${canLock?"":"disabled"}>Finalize amounts</button><p class="fine-print center">${canLock?"This locks everyone’s selections and sends final totals.":"Everyone must finish choosing before totals can be finalized."}</p>`:`<p class="fine-print">Unpaid guests are reminded every 24 hours. Reminders stop as soon as they report payment.</p>`}`;}
