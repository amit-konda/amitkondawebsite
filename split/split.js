...
function editorView() {
  ...
  app.innerHTML = `<a class="back-link" href="#/new" data-action="choose-receipt">← Choose another receipt</a><div class="page-head"><div><p class="eyebrow">New split · 2 of 3</p><h1>Check the details.</h1><p class="lede">Fix anything the scanner missed, then add everyone at the table.</p></div></div>
    <form id="bill-form" class="editor-layout"><div class="editor-main">
      ...
      <section class="card card-pad"><div class="section-head"><div><h2>Receipt items</h2><p class="muted small">Quantity, each price, and line total are shown so bundled items don’t get missed.</p></div><button class="btn btn-sm" type="button" data-action="add-item">+ Add item</button></div><div id="item-list">${items.map((x,i) => itemEditor(x,i)).join("")}</div></section>
