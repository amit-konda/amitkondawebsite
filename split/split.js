...
function editorView() {
  ...
  app.innerHTML = `<a class="back-link" href="#/new">← Choose another receipt</a><div class="page-head"><div><p class="eyebrow">New split · 2 of 3</p><h1>Check the details.</h1><p class="lede">Fix anything the scanner missed, then add everyone at the table.</p></div></div>
    <form id="bill-form" class="editor-layout"><div class="editor-main">
      ...
      <section class="card card-pad"><div class="section-head"><div><h2>People at dinner</h2><p class="muted small">They’ll get an individual link after you publish.</p></div><button class="btn btn-sm" type="button" data-action="pick-contact">From contacts</button></div><div id="participant-list" class="participant-list">${people.map(personEditor).join("")}</div><div class="add-person"><input class="input" id="person-name" placeholder="Name" maxlength="80"><input class="input" id="person-phone" type="tel" inputmode="tel" placeholder="Phone number"><button class="btn" type="button" data-action="add-person">Add</button></div><p class="field-hint">Contact access is optional; manual entry always works.</p></section>
...
async function publishBill(event) { const button=event.currentTarget;setBusy(button,true,"Sending invites…");try{ if(!state.demo) await api(`/bills/${encodeURIComponent(state.draft.id)}/publish`,{method:"POST"}); notice("Invites sent. We’ll keep track from here.");state.dashboard=null;go(`bill/${state.draft.id}`);}catch(error){notice(error.message,"error");setBusy(button,false);} }
...
function renderBill(b) {
  ...
  app.innerHTML=`<a class="back-link" href="#/dashboard">← Dashboard</a><section class="card bill-hero"><div><p class="eyebrow">${esc((b.status||"open").replaceAll("_"," "))}</p><h1>${esc(b.merchantName||b.merchant||"Dinner")}</h1><p class="muted">${dateLabel(b.purchasedAt||b.date||b.receiptDate)} · ${people.length} people</p></div><div class="bill-total"><small>Receipt total</small><span>${money(b.totalCents)}</span></div></section>
    <div class="split-layout"><section class="card card-pad"><div class="section-head"><div><h2>Items</h2><p class="muted small">${mine?"Select everything you had.":"What everyone claimed."}</p></div></div><div>${items.map(x=>claimItem(x,mine)).join("")||`<p class="muted">No item details available.</p>`}</div>${mine?`<button class="btn btn-primary btn-block" type="button" data-action="save-claims">Save my items</button>`:""}</section>
  ...
  document.querySelector('[data-action="save-claims"]')?.addEventListener("click",()=>saveClaims(b)); document.querySelector('[data-action="report-paid"]')?.addEventListener("click",e=>reportPaid(e,mine)); document.querySelector('[data-action="lock-bill"]')?.addEventListener("click",e=>lockBill(e,b));
...
async function lockBill(event,b){setBusy(event.currentTarget,true,"Finalizing…");try{await api(`/bills/${encodeURIComponent(b.id)}/lock`,{method:"POST"});notice("Final totals sent.");billView(b.id);}catch(error){notice(error.message,"error");setBusy(event.currentTarget,false);}}
