...
function expenseView() { shell(true); app.innerHTML = `<a class="back-link" href="#/dashboard">← Back to dashboard</a><div class="page-head"><div><p class="eyebrow">New expense</p><h1>Split something without a receipt.</h1><p class="lede">Uber rides, concert tickets, deposits, and anything else the table shared.</p></div></div><form id="expense-form" class="card card-pad expense-form"><label class="field"><span>Expense name</span><input class="input" name="description" placeholder="Uber to the concert" required maxlength="160"></label><label class="field"><span>Total amount</span><span class="input-prefix"><input class="input money-input" name="amount" inputmode="decimal" placeholder="0.00" required></span></label><fieldset class="field"><legend>How should it be split?</legend><label class="choice"><input type="radio" name="splitMode" value="even" checked> Split evenly</label><label class="choice"><input type="radio" name="splitMode" value="amounts"> Input amounts</label></fieldset><section class="field"><div class="section-head"><div><h2>People at dinner</h2><p class="muted small">Add people here, just like a receipt split.</p></div><button class="btn btn-sm" type="button" data-action="expense-pick-contact">From contacts</button></div><div id="expense-participant-list" class="participant-list"></div><div class="add-person"><div class="contact-name-wrap"><label class="sr-only" for="expense-person-name">Diner name</label><input class="input" id="expense-person-name" placeholder="Search a name" autocomplete="off"><div id="expense-contact-suggestions" class="contact-suggestions" role="listbox" hidden></div></div><input class="input" id="expense-person-phone" type="tel" placeholder="Add phone if no match"><button class="btn" type="button" data-action="expense-add-person">Add</button></div></section><button class="btn btn-primary btn-block" type="submit">Continue</button></form>`; document.querySelector("#expense-form").addEventListener("submit", createExpense); bindExpensePeople(); }
function bindExpensePeople(){const form=document.querySelector("#expense-form"),list=document.querySelector("#expense-participant-list"),name=document.querySelector("#expense-person-name"),phone=document.querySelector("#expense-person-phone"),suggestions=document.querySelector("#expense-contact-suggestions");form.addEventListener("click",event=>{const action=event.target.closest("[data-action]")?.dataset.action;if(action==="expense-add-person"){const displayName=name.value.trim(),rawPhone=phone.value.trim();if(!displayName||phoneKey(rawPhone).length!==10)return notice("Add a name and valid phone number.","error");if([...list.querySelectorAll(".person-phone-value")].some(input=>phoneKey(input.value)===phoneKey(rawPhone)))return notice("That person is already on this split.","error");list.insertAdjacentHTML("beforeend",personEditor({name:displayName,phone:rawPhone}));name.value="";phone.value="";}if(action==="remove-person")event.target.closest(".person")?.remove();if(action==="expense-pick-contact")navigator.contacts?.select?navigator.contacts.select(["name","tel"],{multiple:true}).then(selected=>selected.forEach(contact=>{const n=contact.name?.[0]||"Guest",p=contact.tel?.[0]||"";if(phoneKey(p).length===10&&!([...list.querySelectorAll(".person-phone-value")].some(input=>phoneKey(input.value)===phoneKey(p))))list.insertAdjacentHTML("beforeend",personEditor({name:n,phone:p}));})).catch(()=>{}):notice("Contact picking isn’t available in this browser.","error");});name.addEventListener("input",()=>{const q=name.value.trim();if(!q){suggestions.hidden=true;return;}api(`/contacts?q=${encodeURIComponent(q)}`).then(result=>{suggestions.innerHTML=(result.contacts||[]).map(c=>`<button type="button" role="option" data-name="${esc(c.name)}" data-phone="${esc(c.phone)}"><strong>${esc(c.name)}</strong><small>${esc(c.phone.slice(-4).padStart(c.phone.length,"•"))}</small></button>`).join("");suggestions.hidden=!result.contacts?.length;}).catch(()=>{});});suggestions.addEventListener("click",event=>{const option=event.target.closest("[role=option]");if(!option)return;list.insertAdjacentHTML("beforeend",personEditor({name:option.dataset.name,phone:option.dataset.phone}));name.value="";phone.value="";suggestions.hidden=true;});}
async function createExpense(event) { event.preventDefault(); const form=event.currentTarget, description=String(form.elements.description.value).trim(), totalCents=parseCents(form.elements.amount.value), mode=form.elements.splitMode.value; const participants=[...document.querySelectorAll("#expense-participant-list .person")].map(row=>({name:row.querySelector(".person-name-value").value,phone:row.querySelector(".person-phone-value").value})); if(!description||totalCents<=0)return notice("Enter an expense name and amount.","error"); const phones=participants.map(p=>phoneKey(p.phone));if(phones.some((p,i)=>p.length!==10||phones.indexOf(p)!==i))return notice("Each person needs a unique 10-digit phone number.","error"); if(mode==="even"&&!participants.length)return notice("Add at least one person to split this expense.","error"); state.expenseMode=mode; try { if(state.demo){state.draft={...demoDraft(),id:"demo-expense",expenseType:"manual",merchantName:description,items:[{id:"expense-item",description,quantity:1,unitPriceCents:totalCents,lineTotalCents:totalCents}],subtotalCents:totalCents,taxCents:0,tipCents:0,totalCents,participants};return mode==="even"?billView("demo-expense"):editorView();} const created=await api("/bills",{method:"POST",body:{requestKey:crypto.randomUUID(),merchantName:description,purchasedAt:new Date().toISOString(),subtotalCents:totalCents,taxCents:0,tipCents:0,feeCents:0,discountCents:0,totalCents}}); const bill=normalizeBill(created); await api(`/bills/${encodeURIComponent(bill.id)}/items`,{method:"POST",body:{description,quantity:1,unitPriceCents:totalCents,lineTotalCents:totalCents,displayOrder:0}}); for(const person of participants)await api(`/bills/${encodeURIComponent(bill.id)}/participants`,{method:"POST",body:{displayName:person.name,phone:person.phone}}); if(mode==="even"){await api(`/bills/${encodeURIComponent(bill.id)}/publish`,{method:"POST"});await api(`/bills/${encodeURIComponent(bill.id)}/even-split`,{method:"POST"});notice("Expense split evenly and invites sent.");return billView(bill.id);} state.draft=normalizeBill(await api(`/bills/${encodeURIComponent(bill.id)}`));state.draft.expenseType="manual"; editorView(); } catch(error){notice(error.message,"error");} }
...
function editorView() {
  ...
  bindEditor(b);
  if (b.expenseType) {
    const aside = document.querySelector(".editor-aside .totals");
    aside?.querySelectorAll(".total-line")?.forEach((row, index) => { if (index === 1 || index === 2) row.remove(); });
    const heading = document.querySelector(".editor-aside h2"); if (heading) heading.textContent = "Expense total";
    const first = aside?.querySelector(".total-line"); if (first) first.querySelector("span").textContent = "Total";
    const grand = aside?.querySelector(".total-line.grand"); if (grand) { grand.querySelector("span").textContent = "Amount per person"; grand.querySelector("strong")?.setAttribute("id", "expense-per-person"); }
  }
}
function itemEditor(x,i) { const lowConfidence = Number(x.ocrConfidenceBasisPoints ?? 10000) < 7500; const quantity = Math.max(1, Number(x.quantity || 1)); const lineTotalCents = Number(x.lineTotalCents ?? x.totalCents ?? x.priceCents ?? 0); return `<div class="item-row${lowConfidence ? " low-confidence" : ""}" data-item-id="${esc(x.id || "")}"><input class="input item-name" value="${esc(x.description || x.name)}" aria-label="Item ${i+1} name" maxlength="160"><input class="input item-qty" type="number" min="1" max="1000" step="1" value="${quantity}" aria-label="Item ${i+1} quantity"><span class="input-prefix"><input class="input money-input item-price" value="${(lineTotalCents/100).toFixed(2)}" inputmode="decimal" aria-label="Item ${i+1} line total"></span>${lowConfidence ? `<span class="ocr-flag">Check scan</span>` : ""}<button class="icon-btn" type="button" data-action="remove-item" aria-label="Remove item">×</button></div>`; }
...
function bindEditor(b) {
  ...
  document.querySelector('[data-action="choose-receipt"]')?.addEventListener("click", event => { event.preventDefault(); state.draft = null; state.receiptError = null; go("new"); });
  ...
  const total = () => { const itemSum = [...document.querySelectorAll(".item-price")].reduce((s,x) => s + parseCents(x.value),0); form.elements.subtotal.value = (itemSum/100).toFixed(2); document.querySelector("#calculated-total")?.replaceChildren(money(itemSum + parseCents(form.elements.tax.value) + parseCents(form.elements.tip.value))); const perPerson=document.querySelector("#expense-per-person");if(perPerson){const count=Math.max(1,document.querySelectorAll("#participant-list .person").length);perPerson.textContent=money(Math.ceil(itemSum/count));} };
  form.addEventListener("input", e => { if (e.target.matches(".item-price,[name=tax],[name=tip]")) total(); });
  form.addEventListener("click", e => { const target=e.target.closest("[data-action]"), action = target?.dataset.action; if (action === "add-item") { list.insertAdjacentHTML("beforeend", itemEditor({},list.children.length)); list.lastElementChild.querySelector("input").focus(); } if (action === "remove-item") { e.target.closest(".item-row").remove(); total(); } if (action === "remove-person") e.target.closest(".person").remove(); if (action === "add-person") addPersonFromInputs(people); if (action === "choose-contact") { const chosenName=target.dataset.name||"", chosenPhone=target.dataset.phone||"", key=phoneKey(chosenPhone); const duplicate=[...people.querySelectorAll(".person-phone-value")].some(input=>phoneKey(input.value)===key); if (key.length!==10) return notice("That contact does not have a usable phone number.","error"); if (duplicate) return notice("That person is already on this split.","error"); people.insertAdjacentHTML("beforeend", personEditor({name:chosenName,phone:chosenPhone})); nameInput.value=""; phoneInput.value=""; setSuggestionsOpen(false); notice(`${chosenName} added.`); } if (action === "pick-contact") pickContact(people); });
...
async function saveDraft(event,b) {
  ...
  const items = [...document.querySelectorAll(".item-row")].map((row,index) => { const quantity=Math.max(1,Math.min(1000,Number.parseInt(row.querySelector(".item-qty").value,10)||1)); const lineTotalCents=parseCents(row.querySelector(".item-price").value); return { id:row.dataset.itemId || undefined, description:row.querySelector(".item-name").value.trim(), quantity, unitPriceCents:Math.floor(lineTotalCents/quantity), lineTotalCents, displayOrder:index }; });
  ...
  if ([payload.subtotalCents, payload.taxCents, payload.tipCents, payload.feeCents, payload.discountCents].some(value => value < 0)) return notice("Receipt amounts can’t be negative.", "error");
  const phoneKeys = participants.filter(p => !p.id || p.phone.trim()).map(p => p.phone.replace(/\D/g, "").replace(/^1(?=\d{10}$)/, ""));
...
async function saveItemsAndPeople(b,items,participants) {
  ...
  for(const person of participants) {
    ...
    // Existing invitees may only expose a masked phone number. Preserve
    // their stored phone when no editable number is present.
    if (previous && !person.phone.trim()) continue;
...
function demoBill(id){const participants=[{id:"p1",name:"Alex",userId:id==="demo-2"?"demo-user":"payer-user",paymentStatus:"paid",amountCents:4832,invitationStatus:"accepted"},{id:"p2",name:"Maya",userId:id==="demo-1"||id==="demo-3"?"demo-user":"maya-user",paymentStatus:id==="demo-3"?"confirmed":"unpaid",amountCents:3982,itemSubtotalCents:3200,taxCents:256,tipCents:526,finalAmountCents:3982,invitationStatus:"sent"},{id:"p3",name:"Sam",userId:"sam-user",paymentStatus:"reported_paid",amountCents:2600,invitationStatus:id==="demo-2"?"failed":"sent"}];return {...demoDraft(),id,status:id==="demo-3"?"settled":"open",organizerUserId:id==="demo-2"?"demo-user":"payer-user",participants,currentParticipant:id==="demo-1"||id==="demo-3"?participants[1]:undefined};}
function renderBill(b) {
  ...
  const simpleSplit=b.status==="locked"&&items.length===1&&Number(b.taxCents||0)===0&&Number(b.tipCents||0)===0;
  ...
  app.innerHTML=`<a class="back-link" href="#/dashboard">← Dashboard</a><section class="card bill-hero"><div><p class="eyebrow">${esc((b.status||"open").replaceAll("_"," "))}</p><h1>${esc(b.merchantName||b.merchant||"Dinner")}</h1><p class="muted">${dateLabel(b.purchasedAt||b.date||b.receiptDate)} · ${people.length} people</p></div><div class="bill-total"><small>Receipt total</small><span>${money(b.totalCents)}</span></div></section>
    ${isOrganizer?`<section class="card card-pad retro-add"><div class="section-head"><div><h2>Add people</h2><p class="muted small">Add a diner after creating the receipt. They’ll get an invite text right away.</p></div><div class="section-actions"><button class="btn btn-sm" type="button" data-action="sync-contacts">Sync contacts</button><button class="text-btn btn-danger" type="button" data-action="delete-bill">Delete receipt</button></div></div><div class="add-person"><div class="contact-name-wrap"><label class="sr-only" for="retro-person-name">Diner name</label><input class="input" id="retro-person-name" placeholder="Search a name" autocomplete="off" role="combobox" aria-controls="retro-contact-suggestions" aria-expanded="false"><div id="retro-contact-suggestions" class="contact-suggestions" role="listbox" hidden></div></div><input class="input" id="retro-person-phone" type="tel" placeholder="Add phone if no match"><button class="btn" type="button" data-action="retro-add-person">Add</button></div></section>`:""}
  ...
  document.querySelectorAll('[data-action="mark-paid"]').forEach(button=>button.addEventListener("click",async()=>{if(!window.confirm(`Mark ${button.dataset.name} as paid?`))return;await setPaymentStatus({currentTarget:button},button.dataset.participant,"confirmed",b);}));
  document.querySelectorAll('[data-action="retry-invite"]').forEach(button=>button.addEventListener("click",e=>retryInvite(e,button.dataset.participant,b)));
  document.querySelectorAll('[data-action="remove-participant"]').forEach(button=>button.addEventListener("click",async()=>{if(!window.confirm(`Remove ${button.dataset.name} from this split?`))return;try{await api(`/participants/${encodeURIComponent(button.dataset.participant)}`,{method:"DELETE"});notice("Person removed.");billView(b.id);}catch(error){notice(error.message,"error");}}));
  bindRetroactiveAdd(b);
  if (simpleSplit) { const card=app.querySelector(".split-layout > section.card"); if (card) card.innerHTML=`<h2>Split amount</h2><div class="pay-amount">${money(b.totalCents)}</div><p class="muted">Each person’s amount is shown in payment status.</p>`; }
}
  function bindRetroactiveAdd(b){const name=document.querySelector("#retro-person-name"),phone=document.querySelector("#retro-person-phone"),suggestions=document.querySelector("#retro-contact-suggestions"); if(!name)return; let timer; name.addEventListener("input",()=>{clearTimeout(timer); const q=name.value.trim(); if(!q){suggestions.hidden=true;return;} timer=setTimeout(async()=>{const found=(await api(`/contacts?q=${encodeURIComponent(q)}`).catch(()=>({contacts:[]}))).contacts||[]; suggestions.innerHTML=found.map(c=>`<button type="button" role="option" data-name="${esc(c.name)}" data-phone="${esc(c.phone)}"><strong>${esc(c.name)}</strong><small>${esc(c.phone.slice(-4).padStart(c.phone.length,"•"))}</small></button>`).join(""); suggestions.hidden=!found.length;},180);}); suggestions.addEventListener("click",e=>{const option=e.target.closest("[role=option]");if(!option)return;name.value=option.dataset.name||"";phone.value=option.dataset.phone||"";suggestions.hidden=true;document.querySelector('[data-action="retro-add-person"]')?.click();}); document.querySelector('[data-action="retro-add-person"]')?.addEventListener("click",async()=>{const displayName=name.value.trim(), rawPhone=phone.value.trim(); if(!displayName||phoneKey(rawPhone).length!==10)return notice("Choose an existing person or add a valid phone number.","error"); setBusy(document.querySelector('[data-action="retro-add-person"]'),true,"Adding…"); try{await api(`/bills/${encodeURIComponent(b.id)}/participants`,{method:"POST",body:{displayName,phone:rawPhone}});notice("Person added and invite sent.");billView(b.id);}catch(error){notice(error.message,"error");setBusy(document.querySelector('[data-action="retro-add-person"]'),false);}}); document.querySelector('[data-action="sync-contacts"]')?.addEventListener("click",async event=>{if(!navigator.contacts?.select)return notice("Contact syncing isn’t available in this browser.","error");try{const selected=await navigator.contacts.select(["name","tel"],{multiple:true});let added=0;for(const contact of selected){const displayName=contact.name?.[0]||"Guest",rawPhone=contact.tel?.[0]||"";if(phoneKey(rawPhone).length!==10)continue;try{await api(`/bills/${encodeURIComponent(b.id)}/participants`,{method:"POST",body:{displayName,phone:rawPhone}});added++;}catch(_){}}notice(added?`${added} contact${added===1?"":"s"} added.`:"No new contacts added.");billView(b.id);}catch(_){}}); document.querySelector('[data-action="delete-bill"]')?.addEventListener("click",async()=>{if(!window.confirm("Delete this receipt and all of its split details? This can’t be undone."))return; try{await api(`/bills/${encodeURIComponent(b.id)}`,{method:"DELETE"});state.dashboard=null;notice("Receipt deleted.");go("dashboard");}catch(error){notice(error.message,"error");}});}
function claimItem(x,mine){const assigned=x.allocations||x.claims||[];const selected=assigned.some(a=>a.participantId===mine?.id);const quantity=Math.max(1,Number(x.quantity||1));const lineTotal=x.lineTotalCents??x.totalCents??x.priceCents;const unit=x.unitPriceCents??Math.floor(Number(lineTotal)/quantity);const quantityNote=quantity>1?`<small class="muted">${quantity} × ${money(unit)} each</small>`:"";return `<label class="claim-item">${mine?`<input class="claim-check" type="checkbox" value="${esc(x.id)}" ${selected?"checked":""}>`:`<span class="person-dot">${assigned.length?"✓":"—"}</span>`}<span><strong>${esc(x.description||x.name)}</strong>${quantityNote}<small class="muted">${assigned.length?`${assigned.length} ${assigned.length===1?"person":"people"} claiming`:"Available"}</small></span><span class="claim-price">${money(lineTotal)}</span></label>`;}
...
function organizerStatus(people,b){const canLock=b.status==="open"&&people.length&&people.every(p=>p.selectionStatus==="complete");return `<p class="eyebrow">Payment status</p><h2>Who’s settled up</h2><div class="participant-list">${people.map(p=>{const name=p.displayName||p.name||"Guest",status=p.paymentStatus||"unpaid",invite=p.invitationStatus||"pending",inviteNote=invite==="failed"?"Invite failed":invite==="queued"?"Invite queued":invite==="sent"||invite==="viewed"||invite==="accepted"?"Invite sent":"Invite pending";const action=invite==="failed"&&["open","locked"].includes(b.status)?`<button class="btn btn-sm" type="button" data-action="retry-invite" data-participant="${esc(p.id)}">Retry invite</button>`:b.status==="locked"&&status!=="confirmed"?`<button class="btn btn-sm" type="button" data-action="confirm-payment" data-participant="${esc(p.id)}">${status==="reported_paid"?"Confirm":"Mark paid"}</button>`:b.status==="open"&&p.userId!==b.organizerUserId?`<span class="action-group"><button class="btn btn-sm" type="button" data-action="mark-paid" data-participant="${esc(p.id)}" data-name="${esc(name)}">Mark paid</button><button class="text-btn btn-danger" type="button" data-action="remove-participant" data-participant="${esc(p.id)}" data-name="${esc(name)}">Remove</button></span>`:`<span class="status ${esc(status)}">${esc(status.replaceAll("_"," "))}</span>`;return `<div class="person"><span class="person-dot">${esc(initials(name))}</span><span class="person-copy"><strong>${esc(name)}</strong><small>${p.finalAmountCents!=null?money(p.finalAmountCents):esc((p.selectionStatus||"waiting").replaceAll("_"," "))} · ${esc(inviteNote)}</small></span>${action}</div>`;}).join("")||`<p class="muted">Waiting for guests.</p>`}</div>${b.status==="open"?`<button class="btn btn-dark btn-block" type="button" data-action="lock-bill" ${canLock?"":"disabled"}>Finalize amounts</button><p class="fine-print center">${canLock?"This locks everyone’s selections and sends final totals.":"Everyone must finish choosing before totals can be finalized."}</p>`:`<p class="fine-print">Unpaid guests are reminded every 24 hours. Reminders stop as soon as they report payment.</p>`}`;}
...
async function retryInvite(event,participantId,b){setBusy(event.currentTarget,true,"Retrying…");try{if(state.demo){const participant=(b.participants||[]).find(p=>p.id===participantId);if(participant)participant.invitationStatus="sent";notice("Invite resent.");renderBill(b);return;}const result=await api(`/participants/${encodeURIComponent(participantId)}/retry-invite`,{method:"POST"});notice(result.delivery?.failed?"Invite queued for another retry.":"Invite resent.",result.delivery?.failed?"error":"success");billView(b.id);}catch(error){notice(error.message,"error");setBusy(event.currentTarget,false);}}
async function saveClaims(b,mine){const ids=[...document.querySelectorAll(".claim-check:checked")].map(x=>x.value);const participant=state.invitation?.participant||mine||b.currentParticipant;try{if(!participant?.id&&!state.demo)throw new Error("This invitation is missing a participant.");if(!state.demo){await api(`/participants/${encodeURIComponent(participant.id)}/allocations`,{method:"POST",body:{allocations:ids.map(itemId=>({itemId,kind:"equal_share",shareUnits:1}))}});await api(`/participants/${encodeURIComponent(participant.id)}/selection/complete`,{method:"POST"});}if(participant)participant.selectionStatus="complete";if(state.invitation?.participant)state.invitation.participant.selectionStatus="complete";notice("Your items are saved.");renderBill({...b,currentParticipant:participant,participants:(b.participants||[]).map(p=>p.id===participant?.id?{...p,selectionStatus:"complete"}:p)});}catch(error){notice(error.message,"error");}}
...
async function activityView(){shell(true);app.innerHTML=`<div class="page-head"><div><p class="eyebrow">Activity</p><h1>Recent activity</h1><p class="lede">Your receipts, shares, and payment updates in one place.</p></div></div><div class="bill-list"><div class="skeleton"></div><div class="skeleton"></div></div>`;try{if(!state.dashboard)state.dashboard=state.demo?demoDashboard:await api("/dashboard");const bills=normalizeBills(state.dashboard);app.innerHTML=`<div class="page-head"><div><p class="eyebrow">Activity</p><h1>Recent activity</h1><p class="lede">Your receipts, shares, and payment updates in one place.</p></div></div><section class="section"><div class="section-head"><h2>All activity</h2>${bills.length?`<span class="muted small">${bills.length} receipt${bills.length===1?"":"s"}</span>`:""}</div>${bills.length?`<div class="bill-list">${bills.map(billRow).join("")}</div>`:emptyDashboard()}</section>`;bindCommon();}catch(error){renderError("We couldn’t load your activity.",error.message,activityView);}}
...
function bindCommon(){document.querySelectorAll('[data-action="new-bill"]').forEach(x=>x.addEventListener("click",()=>{state.draft=null;go("new");}));document.querySelectorAll('[data-action="new-expense"]').forEach(x=>x.addEventListener("click",()=>{state.draft=null;go("expense");}));}

function route(){const parts=getRoute();const routeName=parts[0]|| (state.me?"dashboard":"welcome");document.querySelectorAll("[data-nav]").forEach(x=>x.classList.toggle("active",x.dataset.nav===routeName));if(!state.me&&routeName!=="invite")return authView();if(routeName==="welcome")return state.me?go("dashboard"):authView();if(routeName==="dashboard")return dashboardView();if(routeName==="activity")return activityView();if(routeName==="profile")return profileView();if(routeName==="new")return uploadView();if(routeName==="expense")return expenseView();if(routeName==="edit")return editorView();if(routeName==="preview")return previewBill();if(routeName==="bill"&&parts[1])return billView(parts[1]);if(routeName==="invite"&&parts[1])return inviteView(parts[1]);go(state.me?"dashboard":"welcome");}
